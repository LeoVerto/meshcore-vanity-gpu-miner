...
void __global__ vanity_scan(curandState *state, int *keys_found, int *gpu,
...
unsigned long long int makeSeed(bool allow_insecure = false) {
  ...
  try {
    ...
    insecure = true;
    std::cerr << "WARNING: Cryptographically secure random_device could not be "
                 "accessed. Falling back to internal clock for seed "
                 "generation. Seeds will be very insecure and should NOT be "
                 "used for real cryptographic purposes!"
              << std::endl;
  }
  ...
  seed ^= time_seed;
  // Final check: if seed is still 0, just use the time. This should be
  // extremely rare.
  if (seed == 0) {
    seed = time_seed;
  }
  if (insecure && !allow_insecure) {
    std::cerr << "ERROR: Insecure seed source detected and --allow-insecure "
                 "not set. Aborting."
              << std::endl;
    exit(1);
  }
...
void vanity_setup(config &vanity, bool allow_insecure) {
  ...
  for (int i = 0; i < gpuCount; ++i) {
    ...
    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, vanity_scan, 0,
                                       0);
    ...
    cudaMemcpy(dev_rseed, &rseed, sizeof(unsigned long long int),
...
void vanity_run(config &vanity) {
  ...
  for (int i = 0; i < MAX_ITERATIONS; ++i) {
    ...
    for (int g = 0; g < gpuCount; ++g) {
      cudaMemcpy(&keys_found_this_iteration, dev_keys_found[g], sizeof(int),
                 cudaMemcpyDeviceToHost);
      keys_found_total += keys_found_this_iteration;
      printf("GPU %d found %d keys\n", g, keys_found_this_iteration);

      cudaMemcpy(&executions_this_gpu, dev_executions_this_gpu[g],
    ...
    printf("%s Iteration %d Attempts: %llu in %f at %fcps - Total Attempts "
           "%llu - keys found %d\n",
           getTimeStr().c_str(), i + 1, executions_this_iteration,
           elapsed.count(), executions_this_iteration / elapsed.count(),
...
void __global__ vanity_scan(curandState *state, int *keys_found, int *gpu,
  ...
  // unsigned int report_interval = ATTEMPTS_PER_EXECUTION / 10; // Report 10
  ...
  for (int attempts = 0; attempts < ATTEMPTS_PER_EXECUTION; ++attempts) {
    ...
    //
    // This means:
    //   * Normally we iterate for each 128 bytes of input, but we are always <
    //   128. So no iteration.
    //   * We can eliminate a MIN(inlen, (128 - md.curlen)) comparison,
    //   specialize to 32, branch prediction improvement.
    //   * We can eliminate the in/inlen tracking as we will never subtract
    //   while under 128
    //   * As a result, the only thing update does is copy the bytes into the
    //   buffer.
    ...
    //
    // This means:
    //   * We don't need to care about the curlen > 112 check. Eliminating a
    //   branch.
    //   * We only need to run one round of sha512_compress, so we can inline it
    //   entirely as we don't need to unroll.
